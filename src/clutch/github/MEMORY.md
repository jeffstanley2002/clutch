...
GitHub ingestion is GET-only and bounded. URL parsing accepts HTTPS
`github.com` repo/PR URLs, tolerating copied-browser query strings/fragments
while still rejecting credentials, ports, unsupported paths such as
`/tree/...`, and unsafe path segments. The API client targets fixed
`api.github.com`, rejects redirects/oversized bodies, applies short timeouts,
and never returns provider bodies or tokens in errors.
...
Handled MCP tool failures are preserved as `GitHubMcpToolError`. The FastAPI
boundary converts unreadable repo/PR cases, including GitHub 404s from bad,
private, or token-inaccessible URLs, into a clear 422 instead of a generic 500.
