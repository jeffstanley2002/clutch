...
from clutch.schemas import CodeFinding, InterviewTurnRequest, ReviewRequest

DATASET_VERSION = "2026-09-08.v3"
FIXTURE_ROOT = Path(__file__).resolve().parents[3] / "evals" / "fixtures"
MIN_SCORE = 1.0
MIN_RETRIEVAL_RECALL_AT_3 = 0.60
...
async def run_evaluation_suite() -> EvalReport:
    ...
    severity_matches = sum(result.severity_matches for result in case_results)
    retrieval_results = [
        result for result in case_results if result.retrieval_evaluated
    ]
    ...
    finding_count = sum(result.predicted_findings for result in case_results)
    hallucinated_lines = sum(
        result.hallucinated_line_numbers for result in case_results
    )
    clean_results = [result for result in case_results if result.kind == "clean"]
    mixed_results = [result for result in case_results if result.kind == "mixed"]
    ...
    retrieval_ndcg = _ratio(
        sum(result.retrieval_ndcg for result in case_results),
        len(case_results),
    )
    all_retrieved = sum(result.retrieval_returned for result in case_results)
    retrieval_judgment_coverage = _ratio(
        sum(result.retrieval_judged for result in case_results),
        all_retrieved,
    )
    retrieval_irrelevant = _ratio(
        sum(result.retrieval_irrelevant for result in case_results),
        all_retrieved,
    )
    citation_faithfulness = _ratio(
        sum(result.citation_faithful for result in case_results),
        len(case_results),
    )
    question_relevance = _ratio(
        sum(result.question_relevant for result in case_results),
        len(case_results),
    )
    ...
    return EvalReport(
        ...
        average_latency_ms=sum(result.latency_ms for result in case_results)
        / len(case_results),
...
async def _evaluate_golden_case(
    ...
    findings: list[CodeFinding] = state["findings"]
